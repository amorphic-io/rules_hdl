...
load(
    ...
    SKYWATER_PDK_CELL_LIBRARIES = "CELL_LIBRARIES"
)
load(
    "//dependency_support/com_google_gf180mcu_pdk:cell_libraries.bzl",
    GS_PDK_CELL_LIBRARIES = "CELL_LIBRARIES",
)
...
def register_skywater_pdk(module_ctx, module, pdk, root_direct_deps, root_direct_dev_deps, pdk_repo_names):
    for library_name in pdk.libraries:
        ...
        if name not in pdk_repo_names:
            ...
            git_repository(
                name = name,
                commit = SKYWATER_PDK_CELL_LIBRARIES[library_name]["commit"],
                remote = "https://foss-eda-tools.googlesource.com/skywater-pdk/libs/%s.git" % library_name,
                shallow_since = SKYWATER_PDK_CELL_LIBRARIES[library_name]["shallow_since"],
                build_file_content = _GOOGLE_SKYWATER_PDK_BUILD_TEMPLATE.format(name, library_name),
                patches = SKYWATER_PDK_CELL_LIBRARIES[library_name].get("patches", []),
...
_GOOGLE_GS_PDK_BUILD_TEMPLATE = """
load("@rules_hdl//dependency_support/com_google_gf180mcu_pdk:declare_cell_library.bzl", "declare_cell_library")
declare_cell_library("{}", "{}")
"""

def register_gf_pdk(module_ctx, module, pdk, root_direct_deps, root_direct_dev_deps, pdk_repo_names):
    for library_name in pdk.libraries:
        name = pdk.name + "_" + library_name
        if module.is_root:
            if module_ctx.is_dev_dependency(pdk):
        ...
        if name not in pdk_repo_names:
            ...
            git_repository(
                name = name,
                commit = GS_PDK_CELL_LIBRARIES[library_name]["commit"],
                remote = "https://github.com/google/globalfoundries-pdk-libs-%s.git" % library_name,
                shallow_since = GS_PDK_CELL_LIBRARIES[library_name]["shallow_since"],
                build_file_content = _GOOGLE_GS_PDK_BUILD_TEMPLATE.format(name, library_name),
                patches = GS_PDK_CELL_LIBRARIES[library_name].get("patches", []),
                # Strip one directory level. Useful for patches generated by git.
                patch_args = [
                    "-p1",
                ],
...
def _hdl_ext_impl(module_ctx):
    ...
    http_archive(
        name = "com_google_gf180mcu_pdk",
        build_file = "@rules_hdl//dependency_support/com_google_gf180mcu_pdk:bundled.BUILD.bazel",
        sha256 = "b5847f93e55debb49d03ec581e22eb301109ff90c9ad19d35ae1223casd",
        strip_prefix = "gf180mcu-pdk-1ff7649bbf423207f6f70293dc1cf630cd477365",
        urls = [
            "https://github.com/google/gf180mcu-pdk/archive/de3240d7529a6970437ac3344820aaae7839f215.tar.gz",
        ],
    )

    root_direct_deps.append("com_google_gf180mcu_pdk")
    for module in module_ctx.modules:
        for pdk in module.tags.gf_pdk:
            register_gf_pdk(
                module_ctx,
                module,
                pdk,
